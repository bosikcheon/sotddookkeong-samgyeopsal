...
export async function getReservationsByPhone(phone) { const normalized = phone.replace(/\D/g, ''); const hyphenated = normalized.replace(/^(01\d)(\d{4})(\d{4})$/, '$1-$2-$3'); const values = [...new Set([normalized, hyphenated, phone.trim()])]; const responses = await Promise.all(values.map(value => read('reservation', { orderBy: '"r_tel"', equalTo: `"${value}"` }))); const matched = new Map(); responses.filter(Boolean).forEach(records => Object.entries(records).forEach(([idx, data]) => matched.set(idx, { idx, ...data }))); return [...matched.values()].sort((a, b) => new Date(b.r_date) - new Date(a.r_date)); }
