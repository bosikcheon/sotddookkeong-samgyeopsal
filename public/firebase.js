...
async function requestUrl(path) { const token = await authToken(); return token ? `${endpoint(path)}?auth=${encodeURIComponent(token)}` : endpoint(path); }
async function read(path) { const response = await fetch(await requestUrl(path)); if (!response.ok) throw new Error('Firebase Realtime Database 읽기에 실패했습니다. 규칙을 확인해 주세요.'); return response.json(); }
...
export async function createReservation(data) { const idx = crypto.randomUUID(); const record = { idx, r_name: data.r_name.trim(), r_date: data.r_date, rs_date: new Date().toISOString(), r_number: Number(data.r_number), r_tel: data.r_tel.trim(), r_status: 'r', r_content: data.r_content.trim() }; await write(`reservation/${idx}`, 'PUT', record); }
